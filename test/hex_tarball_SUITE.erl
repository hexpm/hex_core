...
build_tools_test(_Config) ->
    ...
    {ok, {Tarball1, _, _}} = hex_tarball:create(maps:put(<<"files">>, [<<"Makefile">>], Metadata), Contents),
    {ok, #{metadata := #{<<"build_tools">> := [<<"make">>]}}} = hex_tarball:unpack(Tarball1, memory),

    {ok, {Tarball2, _, _}} = hex_tarball:create(maps:put(<<"build_tools">>, [<<"mix">>], Metadata), Contents),
    {ok, #{metadata := #{<<"build_tools">> := [<<"mix">>]}}} = hex_tarball:unpack(Tarball2, memory),

    {ok, {Tarball3, _, _}} = hex_tarball:create(Metadata, Contents),
...
unpack_error_handling_test(_Config) ->
    Metadata = #{<<"name">> => <<"foo">>, <<"version">> => <<"1.0.0">>},
    {ok, {Tarball, InnerChecksum, OuterChecksum}} = hex_tarball:create(Metadata, [{"rebar.config", <<"">>}]),
