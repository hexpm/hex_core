...
-spec encode_msg(map(),'Package' | 'Versions') -> binary().
encode_msg(Msg, MsgName) ->
    encode_msg(Msg, MsgName, []).


-spec encode_msg(map(),'Package' | 'Versions', list()) -> binary().
...
dfp_read_field_def_Package(<<26, Rest/binary>>, Z1, Z2,
			   F1, F2, F3, F4, TrUserData) ->
    d_pfield_Package_retired(Rest, Z1, Z2, F1, F2, F3, F4,
			     TrUserData);
...
dfp_read_field_def_Package(<<>>, 0, 0, F1, F2, F3, F4,
    ...
    S1 = #{name => F1},
    S2 = if F2 == '$undef' -> S1;
	    true -> S1#{versions => lists_reverse(F2, TrUserData)}
	 end,
    S3 = if F3 == '$undef' -> S2;
	    true -> S2#{retired => lists_reverse(F3, TrUserData)}
	 end,
    if F4 == '$undef' -> S3;
       true -> S3#{repository => F4}
    end;
...
dg_read_field_def_Package(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
	  ...
	  d_field_Package_versions(Rest, 0, 0, F1, F2, F3, F4,
				   TrUserData);
      26 ->
	  d_pfield_Package_retired(Rest, 0, 0, F1, F2, F3, F4,
				   TrUserData);
      24 ->
	  ...
	  case Key band 7 of
		...
		skip_length_delimited_Package(Rest, 0, 0, F1, F2, F3,
					      F4, TrUserData);
	    3 ->
		skip_group_Package(Rest, Key bsr 3, 0, F1, F2, F3, F4,
				   TrUserData);
	    5 ->
...
d_field_Package_retired(<<0:1, X:7, Rest/binary>>, N,
    ...
    dfp_read_field_def_Package(Rest, 0, 0, F1, F2,
			       cons(NewFValue, F3, TrUserData), F4, TrUserData).

d_pfield_Package_retired(<<1:1, X:7, Rest/binary>>, N,
			 Acc, F1, F2, F3, F4, TrUserData)
    when N < 57 ->
    d_pfield_Package_retired(Rest, N + 7, X bsl N + Acc, F1,
			     F2, F3, F4, TrUserData);
...
skip_group_Package(Bin, FNum, Z2, F1, F2, F3, F4,
		   TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_Package(Rest, 0, Z2, F1, F2, F3, F4,
			       TrUserData).
...
dfp_read_field_def_Versions(<<>>, 0, 0, F1,
    ...
    S1 = #{},
    if F1 == '$undef' -> S1;
       true -> S1#{packages => lists_reverse(F1, TrUserData)}
    end;
    ...
    case Key of
	  ...
	  case Key band 7 of
		...
		skip_length_delimited_Versions(Rest, 0, 0, F1,
					       TrUserData);
	    3 ->
		skip_group_Versions(Rest, Key bsr 3, 0, F1, TrUserData);
...
skip_group_Versions(Bin, FNum, Z2, F1, TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_Versions(Rest, 0, Z2, F1,
				TrUserData).
