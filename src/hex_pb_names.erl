...
-spec encode_msg(map(),'Package' | 'Names') -> binary().
encode_msg(Msg, MsgName) ->
    encode_msg(Msg, MsgName, []).


-spec encode_msg(map(),'Package' | 'Names', list()) -> binary().
    ...
    case Key of
	  ...
	  case Key band 7 of
		...
		skip_length_delimited_Package(Rest, 0, 0, F1, F2,
					      TrUserData);
	    3 ->
		skip_group_Package(Rest, Key bsr 3, 0, F1, F2,
				   TrUserData);
...
skip_group_Package(Bin, FNum, Z2, F1, F2, TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_Package(Rest, 0, Z2, F1, F2,
			       TrUserData).
...
dg_read_field_def_Names(<<0:1, X:7, Rest/binary>>, N,
    ...
    case Key of
	  ...
	  case Key band 7 of
	    ...
	    3 ->
		skip_group_Names(Rest, Key bsr 3, 0, F1, TrUserData);
...
skip_group_Names(Bin, FNum, Z2, F1, TrUserData) ->
    {_, Rest} = read_group(Bin, FNum),
    dfp_read_field_def_Names(Rest, 0, Z2, F1, TrUserData).
